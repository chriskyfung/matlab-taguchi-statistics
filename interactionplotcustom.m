function  [H, ax, BigAx]  = interactionplot(y,group,fun, varargin)
% Modification by Chris, K.Y. FUNG in Oct 2014
%    New input argument:
%       fun is a function handler of a function that fy = f(y).
%
...
if nargin <3
    error(message('stats:interactionplot:FewInput'))
end
...
if ~isa(fun, 'function_handle')
    error(message('stats:interactionplot:BadFunctionHandle'))
end

% parse parameter/value pairs
args =   {'varnames','full'};
defaults = {'',true};
[varnames,full] =  internal.stats.parseArgs(args,defaults,varargin{:});

if ~iscell(varnames) && ~ischar(varnames)
    error(message('stats:interactionplot:BadVarnames'))
end
...
if ng<2
    error(message('stats:interactionplot:TooFewFactors'))
end
...
if  any(cellfun(@length,group)~=size(y,1))
    error(message('stats:interactionplot:BadGroup'))
end;
...
for i = 1:ng
    if isnumeric(group{i})
        if ~isvector(group{i})
           error(message('stats:multivarichart:BadGroup'))
        end
    end;
...
if ng ~= length(varnames)
    error(message('stats:interactionplot:MismatchVarnameGroup'))
end;
...
if full % full matrix form
        ...
        for j=cols:-1:1,
            ...
            if  i~=j   % off- diagonal are filled with interaction plots
                ...
                handles = plotaninteraction(ybar,group{anotheridx},group{idx},...
                    ...
                    levelnames{anotheridx},levelnames{idx}, fun);
    ...
    for i = 1:ng-1
        for j = i+1:ng
            ...
            plotaninteraction(ybar,group{i},group{j},varnames{i},varnames{j},levelnames{i},levelnames{j},fun);
            xlabel(varnames{i})
...
%-----------------------------------
function   handle = plotaninteraction(y,factor1,factor2,varname1,varname2,levels1,levels2, fun)
...
[interact,num] = grpstats(y,{factor1,factor2}, {@taguchiSNRsmaller,'numel'});  % group means w.r.t the two factors
...
if  length(num) < num1*num2
    warning(message('stats:interactionplot:UnequalLevels'))
    dff = fullfact([num1 num2]);
%     grpdff(:,1) = nominal(dff(:,1),levels1);
%     grpdff(:,2) = nominal(dff(:,2),levels2);
%     tgrpdff = table(grpdff);
%     yy = dataset(y,factor1,factor2, 'VarNames', {'y', varname1,varname2});
%     statarray = grpstats(yy,{varname1,varname2});
%     tstat = dataset2table(statarray(:,1:2));

    [~,ia] = setdiff(dff, [factor1 factor2],'rows');
    for i = 1:length(ia)
       interact(ia(i)+1:end+1) = interact(ia(i):end);
       interact(ia(i)) = NaN;
    end
    if length(interact) < num1*num2
        error(message('stats:interactionplot:UnequalLevels'))
    end
end
